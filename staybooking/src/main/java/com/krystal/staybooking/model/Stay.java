...
public class Stay implements Serializable{
    ...
    //version id 举个简单例子就是 如果你数据的version id已经到了 2L（L是long的意思）然后你收到一个1L的更新， 你就知道这个更新过时了不需要处理，同理如果是3L你就知道要更新了

    @Id //primary key
    @GeneratedValue(strategy = GenerationType.AUTO) //不需要担心要传哪个primary key 自增id
    private Long id;
    private String name;
    ...
    @ManyToOne //stay和user是多对一的关系
    @JoinColumn(name = "user_id")
    private User host;

    @OneToMany(mappedBy = "stay", cascade = CascadeType.ALL, fetch = FetchType.EAGER) //自动取回来
